...
`public_web`인데 경로가 없거나 상대경로면 실행을 거부한다(데모로 조용히 되돌아가지
않는다). 경로만 설정하고 `HARNESS_JOB_SOURCE`를 바꾸지 않으면 **아무 일도 일어나지
않는다** — 활성화는 언제나 명시적 선택이다.
...
| `UNSUPPORTED_CATEGORY` | 카테고리가 데모 7종 분류가 아닐 때(임의로 분류하지 않는다) |
...
4. **카테고리 매핑.** 수집 레인이 데모 7종 분류에 맞춘 `category`를 채워야 한다.
   여기서 임의로 분류하지 않는다.
